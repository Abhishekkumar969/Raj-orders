...
const Room1009 = () => {
    ...
    useEffect(() => {
        const storedOrders = JSON.parse(localStorage.getItem("room1009Orders")) || [];
        setOrders(storedOrders);
    }, []);

    useEffect(() => {
        localStorage.setItem("room1009Orders", JSON.stringify(orders));
    }, [orders]);
    ...
    const submitOrder = async () => {
        ...
        try {
            const roomNumber = "1009"; // Set Room No. Here (Change dynamically if needed)
            ...
            setOrders([]);
            localStorage.removeItem("room1009Orders");
        } catch (error) {
