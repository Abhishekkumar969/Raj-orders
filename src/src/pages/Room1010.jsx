...
const Room1010 = () => {
    ...
    useEffect(() => {
        const storedOrders = JSON.parse(localStorage.getItem("room1010Orders")) || [];
        setOrders(storedOrders);
    }, []);

    useEffect(() => {
        localStorage.setItem("room1010Orders", JSON.stringify(orders));
    }, [orders]);
    ...
    const submitOrder = async () => {
        ...
        try {
            const roomNumber = "1010"; // Set Room No. Here (Change dynamically if needed)
            ...
            setOrders([]);
            localStorage.removeItem("room1010Orders");
        } catch (error) {
