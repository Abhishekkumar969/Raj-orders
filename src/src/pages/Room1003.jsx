...
const Room1003 = () => {
    ...
    useEffect(() => {
        const storedOrders = JSON.parse(localStorage.getItem("room1003Orders")) || [];
        setOrders(storedOrders);
    }, []);

    useEffect(() => {
        localStorage.setItem("room1003Orders", JSON.stringify(orders));
    }, [orders]);
    ...
    const submitOrder = async () => {
        ...
        try {
            const roomNumber = "1003"; // Set Room No. Here (Change dynamically if needed)
            ...
            setOrders([]);
            localStorage.removeItem("room1003Orders");
        } catch (error) {
