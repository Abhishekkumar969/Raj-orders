...
const Room1002 = () => {
    ...
    useEffect(() => {
        const storedOrders = JSON.parse(localStorage.getItem("room1002Orders")) || [];
        setOrders(storedOrders);
    }, []);

    useEffect(() => {
        localStorage.setItem("room1002Orders", JSON.stringify(orders));
    }, [orders]);
    ...
    const submitOrder = async () => {
        ...
        try {
            const roomNumber = "1002"; // Set Room No. Here (Change dynamically if needed)
            ...
            setOrders([]);
            localStorage.removeItem("room1002Orders");
        } catch (error) {
