...
const Room1005 = () => {
    ...
    useEffect(() => {
        const storedOrders = JSON.parse(localStorage.getItem("room1005Orders")) || [];
        setOrders(storedOrders);
    }, []);

    useEffect(() => {
        localStorage.setItem("room1005Orders", JSON.stringify(orders));
    }, [orders]);
    ...
    const submitOrder = async () => {
        ...
        try {
            const roomNumber = "1005"; // Set Room No. Here (Change dynamically if needed)
            ...
            setOrders([]);
            localStorage.removeItem("room1005Orders");
        } catch (error) {
